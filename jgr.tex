...
\todo{Figure of conductivity profiles?}
...
\todo{At high \azm, \Alfven waves are not compressional. If we want to
see waves at $L\about5$, that's where the driving needs to be.}

\todo{We know these waves interact with ring current particles. Proxy
for substorm injection, etc. }

\todo{DFT of DST for June 2013 storm. Bins are \SI{16}{\mHz} wide due to
cadence of one minute. That's about the width of the Pc4 range. Fit of
the pink noise suggests, then, that we might expect a wave in the Pc4
range to contribute something like \SI{e-2}{\nT} to DST. Show the plot!}

\todo{That maps out to a current of \about\SI{1}{\mega\A} in space, or
\about\SI{e-4}{\uA/\m\squared} spread over \about\SI{1}{\RE\squared}. }

\todo{We center just outside the plasmapause. $L\about5$,
\SI{5}{\degree} off the equator, gaussian in $L$ and $\theta$. }

\todo{Driving is sinusoidal, azimuthally directed.}
